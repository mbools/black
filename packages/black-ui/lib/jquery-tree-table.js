...
(function ($) {
    ...
    $.widget("mbools.treetable", {
        ...
        _redecorate: function _redecorate() {
            ...
            var treeCol = this._treeColumn();

            var currentTNode = this._tree.children[0].$row.children('td.jtt-tree-node').index();
            ...
            if (currentTNode >= 0 && currentTNode !== treeCol - 2) {
                ...
                var currNodes = this.element.find('.jtt-tree-node');
            ...
            this._treeWalk(this._tree, function (node, depth) {
                ...
                if (!node.children.length) {
                    ...
                    _this3._appendControlTo(node, connector);
                }

                if (_this3.options.showLines && node.parent && node.parent !== '/') {
         ...
         * @private
         */
        _appendControlTo: function _appendControlTo(node, connector) {
            var self = this;
        ...
        _toggleNode: function _toggleNode(node) {
            ...
            for (var i = 0; i < numChildren; i++) {
                this._treeWalk(node.children[i], function (child) {
                    if (child.open) {
